/*
    SPDX-FileCopyrightText: 2025 izll
    SPDX-License-Identifier: GPL-3.0-or-later
*/

import QtQuick

QtObject {
    id: translations

    property string currentLanguage: "system"
    property int version: 0

    onCurrentLanguageChanged: {
        version++;
    }

    readonly property var strings: ({
        "en_US": {
            "weekly usage": "weekly usage",
            "Token Stats (Today)": "Token Stats (Today)",
            "Claude Usage": "Claude Usage",
            "Session (5hr)": "Session (5hr)",
            "Weekly (7day)": "Weekly (7day)",
            "By Model (Weekly)": "By Model (Weekly)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "Resets at:",
            "Resets:": "Resets:",
            "Updated:": "Updated:",
            "Loading...": "Loading...",
            "Refresh": "Refresh",
            "Not logged in": "Not logged in",
            "Token expired": "Token expired",
            "Re-login required": "Re-login required",
            "API error": "API error",
            "Run 'claude' to log in": "Run 'claude' to log in",
            "Will retry automatically": "Will retry automatically",
            "Network error - showing cached data": "Network error - showing cached data",
            "No model breakdown available": "No model breakdown available",
            "Language:": "Language:",
            "System default": "System default",
            "d": "d",
            "h": "h",
            "m": "m",
            "Refresh interval:": "Refresh interval:",
            "minutes": "minutes",
            "Open Claude": "Open Claude",
            "Rate limited": "Rate limited",
            "Auto-retry in 1 min": "Auto-retry in 1 min",
            "Custom API (optional)": "Custom API (optional)",
            "Base URL:": "Base URL:",
            "API key:": "API key:",
            "Leave empty to use the credentials file (default)": "Leave empty to use the credentials file (default)",
            "Check base URL and API key in widget settings": "Check base URL and API key in widget settings",
            "API key not configured": "API key not configured",
            "Invalid API key": "Invalid API key",
            "Endpoint not found": "Endpoint not found",
            "Values under 5 min may cause rate limiting": "Values under 5 min may cause rate limiting",
            "Panel display": "Panel display",
            "Show in panel:": "Show in panel:",
            "Icon:": "Icon:",
            "Icon size:": "Icon size:",
            "Metrics size (panel and popup):": "Metrics size (panel and popup):",
            "Auto": "Auto",
            "Show Claude icon": "Show Claude icon",
            "Style:": "Style:",
            "Text": "Text",
            "Bar": "Bar",
            "Layout:": "Layout:",
            "Horizontal": "Horizontal",
            "Vertical": "Vertical",
            "Background opacity (desktop):": "Background opacity (desktop):",
            "No model data yet": "No model data yet",
            "Popup cards": "Popup cards",
            "Reset to default": "Reset to default",
            "Session & Weekly": "Session & Weekly",
            "Quick links": "Quick links",
            "Add link": "Add link",
            "Show quick links in popup": "Show quick links in popup",
            "Name": "Name",
            "Visibility": "Visibility",
            "When Claude is not running:": "When Claude is not running:",
            "Always show full widget": "Always show full widget",
            "Hide usage, only show icon": "Hide usage, only show icon",
            "Fully hide widget": "Fully hide widget",
            "Process check interval:": "Process check interval:",
            "seconds": "seconds",
            "Account": "Account",
            "Email": "Email",
            "Plan": "Plan",
            "resets in": "resets in",
            "Extra Usage": "Extra Usage",
            "spent": "spent",
            "Today's Tokens": "Today's Tokens",
            "local logs": "local logs",
            "out": "out",
            "7-day trend": "7-day trend",
            "session usage": "session usage",
            "available": "available",
            "Update": "Update",
            "Edit cards": "Edit cards",
            "Done": "Done",
            "Updated {duration} ago": "Updated {duration} ago",
            "Next update in {duration}": "Next update in {duration}",
            "Other models aren't reported by the API yet": "Other models aren't reported by the API yet",
            "Quota Reset": "Quota Reset",
            "quota has been reset. Claude is ready to use again.": "quota has been reset. Claude is ready to use again.",
            "Usage Notice": "Usage Notice",
            "usage has reached": "usage has reached",
            "Notifications:": "Notifications:",
            "Threshold and reset notifications": "Threshold and reset notifications",
            "Widget style:": "Widget style:",
            "Sections": "Sections",
            "Scrollable content:": "Scrollable content:",
            "Enable scrollable content": "Enable scrollable content",
            "Classic": "Classic",
            "Card": "Card",
            "Panel icon:": "Panel icon:",
            "Ring": "Ring",
            "Update check": "Update check",
            "Check for Claude Code updates": "Check for Claude Code updates",
            "Installations": "Installations",
            "Show Claude Code installations": "Show Claude Code installations",
            "Auto-retry in": "Auto-retry in",
            "Colors": "Colors",
            "Ring center:": "Ring center:",
            "Window labels:": "Window labels:",
            "Show 5h/7d on rings": "Show 5h/7d on rings",
            "Panel margin:": "Panel margin:",
            "Percent": "Percent",
            "Logo": "Logo",
            "Logo + %": "Logo + %",
            "Colors:": "Colors:",
            "Theme": "Theme",
            "Time-proportional warnings": "Time-proportional warnings",
            "Authentication": "Authentication",
            "Credentials path:": "Credentials path:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.",
            "Session refresh:": "Session refresh:",
            "Automatically refresh expired session": "Automatically refresh expired session",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually."
        },
        "hu_HU": {
            "weekly usage": "heti használat",
            "Token Stats (Today)": "Token statisztika (ma)",
            "Claude Usage": "Claude használat",
            "Session (5hr)": "Munkamenet (5 óra)",
            "Weekly (7day)": "Heti (7 nap)",
            "By Model (Weekly)": "Modell szerint (Heti)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "Visszaáll:",
            "Resets:": "Visszaáll:",
            "Updated:": "Frissítve:",
            "Loading...": "Betöltés...",
            "Refresh": "Frissítés",
            "Not logged in": "Nincs bejelentkezve",
            "Token expired": "Lejárt token",
            "Re-login required": "Újra bejelentkezés szükséges",
            "API error": "API hiba",
            "Run 'claude' to log in": "Futtasd a 'claude' parancsot",
            "Will retry automatically": "Automatikus újrapróbálkozás",
            "Network error - showing cached data": "Hálózati hiba - gyorsítótárazott adatok",
            "No model breakdown available": "Nincs modell bontás",
            "Language:": "Nyelv:",
            "System default": "Rendszer alapértelmezett",
            "d": "n",
            "h": "ó",
            "m": "p",
            "Refresh interval:": "Frissítési gyakoriság:",
            "minutes": "perc",
            "Open Claude": "Claude megnyitása",
            "Rate limited": "Korlát elérve",
            "Auto-retry in 1 min": "Automatikus újrapróbálás 1 perc múlva",
            "Custom API (optional)": "Egyéni API (opcionális)",
            "Base URL:": "Alap URL:",
            "API key:": "API kulcs:",
            "Leave empty to use the credentials file (default)": "Hagyd üresen a hitelesítő adatok fájl használatához (alapértelmezett)",
            "Check base URL and API key in widget settings": "Ellenőrizd az alap URL-t és API kulcsot a widget beállításokban",
            "API key not configured": "API kulcs nincs beállítva",
            "Invalid API key": "Érvénytelen API kulcs",
            "Endpoint not found": "Végpont nem található",
            "Values under 5 min may cause rate limiting": "5 perc alatti értékek rate limitet okozhatnak",
            "Panel display": "Panel megjelenítés",
            "Show in panel:": "Megjelenítés a panelen:",
            "Icon:": "Ikon:",
            "Icon size:": "Ikon méret:",
            "Metrics size (panel and popup):": "Mérőszám méret (panel és felugró):",
            "Auto": "Automatikus",
            "Show Claude icon": "Claude ikon megjelenítése",
            "Style:": "Stílus:",
            "Text": "Szöveges",
            "Bar": "Sávdiagram",
            "Layout:": "Elrendezés:",
            "Horizontal": "Vízszintes",
            "Vertical": "Függőleges",
            "Background opacity (desktop):": "Háttér átlátszóság (asztal):",
            "No model data yet": "Még nincs modell adat",
            "Popup cards": "Felugró kártyák",
            "Reset to default": "Alapértelmezés visszaállítása",
            "Session & Weekly": "Munkamenet és heti",
            "Quick links": "Gyorslinkek",
            "Add link": "Link hozzáadása",
            "Show quick links in popup": "Gyorslinkek megjelenítése a felugró ablakban",
            "Name": "Név",
            "Visibility": "Láthatóság",
            "When Claude is not running:": "Ha a Claude nem fut:",
            "Always show full widget": "Mindig mutasd a teljes widgetet",
            "Hide usage, only show icon": "Használat elrejtése, csak ikon",
            "Fully hide widget": "Widget teljes elrejtése",
            "Process check interval:": "Folyamat ellenőrzési időköz:",
            "seconds": "másodperc",
            "Account": "Fiók",
            "Email": "E-mail",
            "Plan": "Csomag",
            "resets in": "visszaáll",
            "Extra Usage": "Extra használat",
            "spent": "elköltve",
            "Today's Tokens": "Mai tokenek",
            "local logs": "helyi naplók",
            "out": "ki",
            "7-day trend": "7 napos trend",
            "session usage": "munkamenet használat",
            "available": "elérhető",
            "Update": "Frissítés",
            "Edit cards": "Kártyák szerkesztése",
            "Done": "Kész",
            "Updated {duration} ago": "{duration} ezelőtt frissítve",
            "Next update in {duration}": "Következő frissítés {duration} múlva",
            "Other models aren't reported by the API yet": "Más modelleket az API még nem jelent",
            "Quota Reset": "Kvóta visszaállítva",
            "quota has been reset. Claude is ready to use again.": "kvóta visszaállt. A Claude újra használható.",
            "Usage Notice": "Használati értesítés",
            "usage has reached": "használat elérte a",
            "Notifications:": "Értesítések:",
            "Threshold and reset notifications": "Küszöb- és visszaállítási értesítések",
            "Widget style:": "Widget stílus:",
            "Sections": "Szekciók",
            "Scrollable content:": "Görgethető tartalom:",
            "Enable scrollable content": "Görgethető tartalom engedélyezése",
            "Classic": "Klasszikus",
            "Card": "Kártya",
            "Panel icon:": "Panel ikon:",
            "Ring": "Gyűrű",
            "Update check": "Frissítés ellenőrzés",
            "Check for Claude Code updates": "Claude Code frissítések keresése",
            "Installations": "Telepítések",
            "Show Claude Code installations": "Claude Code telepítések megjelenítése",
            "Auto-retry in": "Automatikus újrapróbálás",
            "Colors": "Színek",
            "Ring center:": "Gyűrű közepe:",
            "Window labels:": "Ablak feliratok:",
            "Show 5h/7d on rings": "5h/7d a gyűrűkön",
            "Panel margin:": "Panel margó:",
            "Percent": "Százalék",
            "Logo": "Logó",
            "Logo + %": "Logó + %",
            "Colors:": "Színek:",
            "Theme": "Téma",
            "Time-proportional warnings": "Időarányos figyelmeztetések",
            "Authentication": "Hitelesítés",
            "Credentials path:": "Hitelesítő adatok útvonala:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": "A .credentials.json teljes útvonala, vagy az azt tartalmazó mappa. Ugyanez a mappa szolgál a fiókadatokhoz és a token statisztikákhoz. Hagyd üresen a $CLAUDE_CONFIG_DIR használatához, vagy a ~/.claude mappához, ha az nincs beállítva.",
            "Session refresh:": "Munkamenet frissítése:",
            "Automatically refresh expired session": "Lejárt munkamenet automatikus frissítése",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "Röviden újranyitja a claude-ot a háttérben (kb. 15 másodperc, látható ablak nélkül, üzenet küldése nélkül), hogy frissítse a munkamenetet, mielőtt kézi bejelentkezésre kérne."
        },
        "de_DE": {
            "weekly usage": "wöchentliche Nutzung",
            "Colors": "Farben",
            "Token Stats (Today)": "Token-Statistik (heute)",
            "Claude Usage": "Claude Nutzung",
            "Session (5hr)": "Sitzung (5 Std)",
            "Weekly (7day)": "Wöchentlich (7 Tage)",
            "By Model (Weekly)": "Nach Modell (Wöchentlich)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "Setzt zurück um:",
            "Resets:": "Setzt zurück:",
            "Updated:": "Aktualisiert:",
            "Loading...": "Laden...",
            "Refresh": "Aktualisieren",
            "Not logged in": "Nicht angemeldet",
            "Token expired": "Token abgelaufen",
            "Re-login required": "Erneute Anmeldung erforderlich",
            "API error": "API-Fehler",
            "Run 'claude' to log in": "Führen Sie 'claude' aus",
            "Will retry automatically": "Automatische Wiederholung",
            "Network error - showing cached data": "Netzwerkfehler - zwischengespeicherte Daten",
            "No model breakdown available": "Keine Modellaufschlüsselung verfügbar",
            "Language:": "Sprache:",
            "System default": "Systemstandard",
            "d": "T",
            "h": "Std",
            "m": "Min",
            "Refresh interval:": "Aktualisierungsintervall:",
            "minutes": "Minuten",
            "Open Claude": "Claude öffnen",
            "Rate limited": "Rate-Limit erreicht",
            "Auto-retry in 1 min": "Automatische Wiederholung in 1 Min",
            "Custom API (optional)": "Benutzerdefinierte API (optional)",
            "Base URL:": "Basis-URL:",
            "API key:": "API-Schlüssel:",
            "Leave empty to use the credentials file (default)": "Leer lassen, um die Anmeldedaten-Datei zu verwenden (Standard)",
            "Check base URL and API key in widget settings": "Überprüfen Sie Basis-URL und API-Schlüssel in den Widget-Einstellungen",
            "API key not configured": "API-Schlüssel nicht konfiguriert",
            "Invalid API key": "Ungültiger API-Schlüssel",
            "Endpoint not found": "Endpunkt nicht gefunden",
            "Values under 5 min may cause rate limiting": "Werte unter 5 Min können Rate-Limiting verursachen",
            "Panel display": "Panel-Anzeige",
            "Show in panel:": "Im Panel anzeigen:",
            "Icon:": "Symbol:",
            "Icon size:": "Symbolgröße:",
            "Metrics size (panel and popup):": "Metrikgröße (Leiste und Popup):",
            "Auto": "Automatisch",
            "Show Claude icon": "Claude-Symbol anzeigen",
            "Style:": "Stil:",
            "Text": "Text",
            "Bar": "Balken",
            "Layout:": "Layout:",
            "Horizontal": "Horizontal",
            "Vertical": "Vertikal",
            "Background opacity (desktop):": "Hintergrund-Deckkraft (Desktop):",
            "No model data yet": "Noch keine Modelldaten",
            "Popup cards": "Popup-Karten",
            "Reset to default": "Auf Standard zurücksetzen",
            "Session & Weekly": "Sitzung & Wöchentlich",
            "Quick links": "Schnelllinks",
            "Add link": "Link hinzufügen",
            "Show quick links in popup": "Schnelllinks im Popup anzeigen",
            "Name": "Name",
            "Visibility": "Sichtbarkeit",
            "When Claude is not running:": "Wenn Claude nicht läuft:",
            "Always show full widget": "Widget immer vollständig anzeigen",
            "Hide usage, only show icon": "Nutzung ausblenden, nur Symbol zeigen",
            "Fully hide widget": "Widget vollständig ausblenden",
            "Process check interval:": "Prozessprüfintervall:",
            "seconds": "Sekunden",
            "Account": "Konto",
            "Email": "E-Mail",
            "Plan": "Tarif",
            "resets in": "setzt zurück in",
            "Extra Usage": "Zusätzliche Nutzung",
            "spent": "verbraucht",
            "Today's Tokens": "Heutige Tokens",
            "local logs": "lokale Logs",
            "out": "aus",
            "7-day trend": "7-Tage-Trend",
            "session usage": "Sitzungsnutzung",
            "available": "verfügbar",
            "Update": "Aktualisieren",
            "Edit cards": "Karten bearbeiten",
            "Done": "Fertig",
            "Updated {duration} ago": "Vor {duration} aktualisiert",
            "Next update in {duration}": "Nächstes Update in {duration}",
            "Other models aren't reported by the API yet": "Andere Modelle werden von der API noch nicht gemeldet",
            "Quota Reset": "Kontingent zurückgesetzt",
            "quota has been reset. Claude is ready to use again.": "Kontingent wurde zurückgesetzt. Claude ist wieder einsatzbereit.",
            "Usage Notice": "Nutzungshinweis",
            "usage has reached": "Nutzung hat erreicht",
            "Notifications:": "Benachrichtigungen:",
            "Threshold and reset notifications": "Schwellenwert- und Reset-Benachrichtigungen",
            "Widget style:": "Widget-Stil:",
            "Sections": "Abschnitte",
            "Scrollable content:": "Scrollbarer Inhalt:",
            "Enable scrollable content": "Scrollbaren Inhalt aktivieren",
            "Classic": "Klassisch",
            "Card": "Karte",
            "Panel icon:": "Panel-Symbol:",
            "Ring": "Ring",
            "Update check": "Update-Prüfung",
            "Check for Claude Code updates": "Auf Claude Code Updates prüfen",
            "Installations": "Installationen",
            "Show Claude Code installations": "Claude Code Installationen anzeigen",
            "Auto-retry in": "Automatische Wiederholung in",
            "Colors:": "Farben:",
            "Theme": "Thema",
            "Time-proportional warnings": "Zeitproportionale Warnungen",
            "Authentication": "Authentifizierung",
            "Credentials path:": "Anmeldedaten-Pfad:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": "Vollständiger Pfad zu .credentials.json oder der Ordner, der sie enthält. Derselbe Ordner wird für Kontoinformationen und Token-Statistiken verwendet. Leer lassen für $CLAUDE_CONFIG_DIR bzw. ~/.claude, wenn diese nicht gesetzt ist.",
            "Session refresh:": "Sitzungsaktualisierung:",
            "Automatically refresh expired session": "Abgelaufene Sitzung automatisch aktualisieren",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "Öffnet claude kurz im Hintergrund (etwa 15 Sekunden, kein sichtbares Fenster, keine Nachricht gesendet), um die Sitzung zu aktualisieren, bevor Sie zur manuellen Anmeldung aufgefordert werden."
        },
        "fr_FR": {
            "weekly usage": "utilisation hebdomadaire",
            "Colors": "Couleurs",
            "Token Stats (Today)": "Statistiques de tokens (aujourd'hui)",
            "Claude Usage": "Utilisation Claude",
            "Session (5hr)": "Session (5h)",
            "Weekly (7day)": "Hebdomadaire (7j)",
            "By Model (Weekly)": "Par modèle (Hebdo)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "Réinitialisation à:",
            "Resets:": "Réinitialisation:",
            "Updated:": "Mis à jour:",
            "Loading...": "Chargement...",
            "Refresh": "Actualiser",
            "Not logged in": "Non connecté",
            "Token expired": "Token expiré",
            "Re-login required": "Reconnexion requise",
            "API error": "Erreur API",
            "Run 'claude' to log in": "Exécutez 'claude' pour vous connecter",
            "Will retry automatically": "Nouvelle tentative automatique",
            "Network error - showing cached data": "Erreur réseau - données en cache",
            "No model breakdown available": "Pas de détail par modèle",
            "Language:": "Langue:",
            "System default": "Système par défaut",
            "d": "j",
            "h": "h",
            "m": "min",
            "Refresh interval:": "Intervalle de rafraîchissement:",
            "minutes": "minutes",
            "Open Claude": "Ouvrir Claude",
            "Rate limited": "Limite atteinte",
            "Auto-retry in 1 min": "Nouvel essai auto dans 1 min",
            "Custom API (optional)": "API personnalisée (optionnel)",
            "Base URL:": "URL de base:",
            "API key:": "Clé API:",
            "Leave empty to use the credentials file (default)": "Laisser vide pour utiliser le fichier d'identifiants (par défaut)",
            "Check base URL and API key in widget settings": "Vérifiez l'URL de base et la clé API dans les paramètres",
            "API key not configured": "Clé API non configurée",
            "Invalid API key": "Clé API invalide",
            "Endpoint not found": "Point d'accès introuvable",
            "Values under 5 min may cause rate limiting": "Les valeurs inférieures à 5 min peuvent causer un rate limiting",
            "Panel display": "Affichage du panneau",
            "Show in panel:": "Afficher dans le panneau:",
            "Icon:": "Icône:",
            "Icon size:": "Taille de l'icône:",
            "Metrics size (panel and popup):": "Taille des métriques (panneau et fenêtre):",
            "Auto": "Automatique",
            "Show Claude icon": "Afficher l'icône Claude",
            "Style:": "Style:",
            "Text": "Texte",
            "Bar": "Barre",
            "Layout:": "Disposition:",
            "Horizontal": "Horizontal",
            "Vertical": "Vertical",
            "Background opacity (desktop):": "Opacité du fond (bureau):",
            "No model data yet": "Pas encore de données de modèle",
            "Popup cards": "Cartes popup",
            "Reset to default": "Réinitialiser par défaut",
            "Session & Weekly": "Session et hebdomadaire",
            "Quick links": "Liens rapides",
            "Add link": "Ajouter un lien",
            "Show quick links in popup": "Afficher les liens rapides dans le popup",
            "Name": "Nom",
            "Visibility": "Visibilité",
            "When Claude is not running:": "Quand Claude ne tourne pas:",
            "Always show full widget": "Toujours afficher le widget complet",
            "Hide usage, only show icon": "Masquer l'utilisation, afficher l'icône uniquement",
            "Fully hide widget": "Masquer complètement le widget",
            "Process check interval:": "Intervalle de vérification du processus:",
            "seconds": "secondes",
            "Account": "Compte",
            "Email": "E-mail",
            "Plan": "Forfait",
            "resets in": "réinitialise dans",
            "Extra Usage": "Usage supplémentaire",
            "spent": "dépensé",
            "Today's Tokens": "Tokens du jour",
            "local logs": "logs locaux",
            "out": "sortie",
            "7-day trend": "Tendance 7 jours",
            "session usage": "usage de session",
            "available": "disponible",
            "Update": "Mettre à jour",
            "Edit cards": "Modifier les cartes",
            "Done": "Terminé",
            "Updated {duration} ago": "Mis à jour il y a {duration}",
            "Next update in {duration}": "Prochaine mise à jour dans {duration}",
            "Other models aren't reported by the API yet": "Les autres modèles ne sont pas encore signalés par l'API",
            "Quota Reset": "Quota réinitialisé",
            "quota has been reset. Claude is ready to use again.": "le quota a été réinitialisé. Claude est prêt à être utilisé.",
            "Usage Notice": "Avis d'utilisation",
            "usage has reached": "l'utilisation a atteint",
            "Notifications:": "Notifications :",
            "Threshold and reset notifications": "Notifications de seuil et réinitialisation",
            "Widget style:": "Style du widget :",
            "Sections": "Sections",
            "Scrollable content:": "Contenu défilable :",
            "Enable scrollable content": "Activer le contenu défilable",
            "Classic": "Classique",
            "Card": "Carte",
            "Panel icon:": "Icône du panneau :",
            "Ring": "Anneau",
            "Update check": "Vérification des mises à jour",
            "Check for Claude Code updates": "Vérifier les mises à jour de Claude Code",
            "Installations": "Installations",
            "Show Claude Code installations": "Afficher les installations Claude Code",
            "Auto-retry in": "Nouvel essai auto dans",
            "Colors:": "Couleurs :",
            "Theme": "Thème",
            "Time-proportional warnings": "Avertissements proportionnels au temps",
            "Authentication": "Authentification",
            "Credentials path:": "Chemin des identifiants:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": "Chemin complet vers .credentials.json, ou le dossier qui le contient. Ce même dossier sert aux informations du compte et aux statistiques de tokens. Laisser vide pour utiliser $CLAUDE_CONFIG_DIR, ou ~/.claude si elle n'est pas définie.",
            "Session refresh:": "Actualisation de session:",
            "Automatically refresh expired session": "Actualiser automatiquement la session expirée",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "Rouvre brièvement claude en arrière-plan (environ 15 secondes, sans fenêtre visible, sans message envoyé) pour actualiser la session avant de vous demander de vous connecter manuellement."
        },
        "es_ES": {
            "weekly usage": "uso semanal",
            "Colors": "Colores",
            "Token Stats (Today)": "Estadísticas de tokens (hoy)",
            "Claude Usage": "Uso de Claude",
            "Session (5hr)": "Sesión (5h)",
            "Weekly (7day)": "Semanal (7 días)",
            "By Model (Weekly)": "Por modelo (Semanal)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "Se reinicia a las:",
            "Resets:": "Se reinicia:",
            "Updated:": "Actualizado:",
            "Loading...": "Cargando...",
            "Refresh": "Actualizar",
            "Not logged in": "No conectado",
            "Token expired": "Token expirado",
            "Re-login required": "Reinicio de sesión requerido",
            "API error": "Error de API",
            "Run 'claude' to log in": "Ejecuta 'claude' para iniciar sesión",
            "Will retry automatically": "Reintento automático",
            "Network error - showing cached data": "Error de red - mostrando datos en caché",
            "No model breakdown available": "Sin desglose por modelo",
            "Language:": "Idioma:",
            "System default": "Predeterminado del sistema",
            "d": "d",
            "h": "h",
            "m": "min",
            "Refresh interval:": "Intervalo de actualización:",
            "minutes": "minutos",
            "Open Claude": "Abrir Claude",
            "Rate limited": "Límite alcanzado",
            "Auto-retry in 1 min": "Reintento automático en 1 min",
            "Custom API (optional)": "API personalizada (opcional)",
            "Base URL:": "URL base:",
            "API key:": "Clave API:",
            "Leave empty to use the credentials file (default)": "Dejar vacío para usar el archivo de credenciales (predeterminado)",
            "Check base URL and API key in widget settings": "Verifica la URL base y la clave API en la configuración del widget",
            "API key not configured": "Clave API no configurada",
            "Invalid API key": "Clave API inválida",
            "Endpoint not found": "Punto de acceso no encontrado",
            "Values under 5 min may cause rate limiting": "Valores menores a 5 min pueden causar rate limiting",
            "Panel display": "Visualización del panel",
            "Show in panel:": "Mostrar en el panel:",
            "Icon:": "Icono:",
            "Icon size:": "Tamaño del icono:",
            "Metrics size (panel and popup):": "Tamaño de las métricas (panel y ventana):",
            "Auto": "Automático",
            "Show Claude icon": "Mostrar icono de Claude",
            "Style:": "Estilo:",
            "Text": "Texto",
            "Bar": "Barra",
            "Layout:": "Disposición:",
            "Horizontal": "Horizontal",
            "Vertical": "Vertical",
            "Background opacity (desktop):": "Opacidad del fondo (escritorio):",
            "No model data yet": "Aún sin datos de modelo",
            "Popup cards": "Tarjetas emergentes",
            "Reset to default": "Restablecer valores predeterminados",
            "Session & Weekly": "Sesión y semanal",
            "Quick links": "Enlaces rápidos",
            "Add link": "Agregar enlace",
            "Show quick links in popup": "Mostrar enlaces rápidos en el popup",
            "Name": "Nombre",
            "Visibility": "Visibilidad",
            "When Claude is not running:": "Cuando Claude no está en ejecución:",
            "Always show full widget": "Mostrar siempre el widget completo",
            "Hide usage, only show icon": "Ocultar uso, mostrar solo icono",
            "Fully hide widget": "Ocultar completamente el widget",
            "Process check interval:": "Intervalo de verificación de proceso:",
            "seconds": "segundos",
            "Account": "Cuenta",
            "Email": "Correo",
            "Plan": "Plan",
            "resets in": "se reinicia en",
            "Extra Usage": "Uso extra",
            "spent": "gastado",
            "Today's Tokens": "Tokens de hoy",
            "local logs": "registros locales",
            "out": "salida",
            "7-day trend": "Tendencia 7 días",
            "session usage": "uso de sesión",
            "available": "disponible",
            "Update": "Actualizar",
            "Edit cards": "Editar tarjetas",
            "Done": "Listo",
            "Updated {duration} ago": "Actualizado hace {duration}",
            "Next update in {duration}": "Próxima actualización en {duration}",
            "Other models aren't reported by the API yet": "Otros modelos aún no son reportados por la API",
            "Quota Reset": "Cuota restablecida",
            "quota has been reset. Claude is ready to use again.": "la cuota se ha restablecido. Claude está listo para usar.",
            "Usage Notice": "Aviso de uso",
            "usage has reached": "el uso ha alcanzado",
            "Notifications:": "Notificaciones:",
            "Threshold and reset notifications": "Notificaciones de umbral y restablecimiento",
            "Widget style:": "Estilo del widget:",
            "Sections": "Secciones",
            "Scrollable content:": "Contenido desplazable:",
            "Enable scrollable content": "Habilitar contenido desplazable",
            "Classic": "Clásico",
            "Card": "Tarjeta",
            "Panel icon:": "Icono del panel:",
            "Ring": "Anillo",
            "Update check": "Verificar actualizaciones",
            "Check for Claude Code updates": "Buscar actualizaciones de Claude Code",
            "Installations": "Instalaciones",
            "Show Claude Code installations": "Mostrar instalaciones de Claude Code",
            "Auto-retry in": "Reintento automático en",
            "Colors:": "Colores:",
            "Theme": "Tema",
            "Time-proportional warnings": "Avisos proporcionales al tiempo",
            "Authentication": "Autenticación",
            "Credentials path:": "Ruta de credenciales:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": "Ruta completa a .credentials.json, o la carpeta que lo contiene. Esa misma carpeta se usa para la información de la cuenta y las estadísticas de tokens. Dejar vacío para usar $CLAUDE_CONFIG_DIR, o ~/.claude si no está definida.",
            "Session refresh:": "Actualización de sesión:",
            "Automatically refresh expired session": "Actualizar automáticamente la sesión expirada",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "Reabre brevemente claude en segundo plano (unos 15 segundos, sin ventana visible, sin enviar mensajes) para actualizar la sesión antes de pedirte que inicies sesión manualmente."
        },
        "it_IT": {
            "weekly usage": "utilizzo settimanale",
            "Colors": "Colori",
            "Token Stats (Today)": "Statistiche token (oggi)",
            "Claude Usage": "Utilizzo Claude",
            "Session (5hr)": "Sessione (5h)",
            "Weekly (7day)": "Settimanale (7gg)",
            "By Model (Weekly)": "Per modello (Settimanale)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "Si azzera alle:",
            "Resets:": "Si azzera:",
            "Updated:": "Aggiornato:",
            "Loading...": "Caricamento...",
            "Refresh": "Aggiorna",
            "Not logged in": "Non connesso",
            "Token expired": "Token scaduto",
            "Re-login required": "Nuovo accesso richiesto",
            "API error": "Errore API",
            "Run 'claude' to log in": "Esegui 'claude' per accedere",
            "Will retry automatically": "Nuovo tentativo automatico",
            "Network error - showing cached data": "Errore di rete - dati dalla cache",
            "No model breakdown available": "Nessun dettaglio per modello",
            "Language:": "Lingua:",
            "System default": "Predefinito di sistema",
            "d": "g",
            "h": "h",
            "m": "min",
            "Refresh interval:": "Intervallo di aggiornamento:",
            "minutes": "minuti",
            "Open Claude": "Apri Claude",
            "Rate limited": "Limite raggiunto",
            "Auto-retry in 1 min": "Riprova automatica tra 1 min",
            "Custom API (optional)": "API personalizzata (opzionale)",
            "Base URL:": "URL base:",
            "API key:": "Chiave API:",
            "Leave empty to use the credentials file (default)": "Lasciare vuoto per usare il file delle credenziali (predefinito)",
            "Check base URL and API key in widget settings": "Controlla URL base e chiave API nelle impostazioni del widget",
            "API key not configured": "Chiave API non configurata",
            "Invalid API key": "Chiave API non valida",
            "Endpoint not found": "Endpoint non trovato",
            "Values under 5 min may cause rate limiting": "Valori sotto 5 min possono causare rate limiting",
            "Panel display": "Visualizzazione pannello",
            "Show in panel:": "Mostra nel pannello:",
            "Icon:": "Icona:",
            "Icon size:": "Dimensione icona:",
            "Metrics size (panel and popup):": "Dimensione metriche (pannello e popup):",
            "Auto": "Automatico",
            "Show Claude icon": "Mostra icona Claude",
            "Style:": "Stile:",
            "Text": "Testo",
            "Bar": "Barra",
            "Layout:": "Disposizione:",
            "Horizontal": "Orizzontale",
            "Vertical": "Verticale",
            "Background opacity (desktop):": "Opacità sfondo (desktop):",
            "No model data yet": "Nessun dato modello ancora",
            "Popup cards": "Schede popup",
            "Reset to default": "Ripristina predefiniti",
            "Session & Weekly": "Sessione e settimanale",
            "Quick links": "Link rapidi",
            "Add link": "Aggiungi link",
            "Show quick links in popup": "Mostra link rapidi nel popup",
            "Name": "Nome",
            "Visibility": "Visibilità",
            "When Claude is not running:": "Quando Claude non è in esecuzione:",
            "Always show full widget": "Mostra sempre il widget completo",
            "Hide usage, only show icon": "Nascondi utilizzo, mostra solo icona",
            "Fully hide widget": "Nascondi completamente il widget",
            "Process check interval:": "Intervallo di controllo processo:",
            "seconds": "secondi",
            "Account": "Account",
            "Email": "Email",
            "Plan": "Piano",
            "resets in": "si azzera tra",
            "Extra Usage": "Utilizzo extra",
            "spent": "speso",
            "Today's Tokens": "Token di oggi",
            "local logs": "log locali",
            "out": "uscita",
            "7-day trend": "Tendenza 7 giorni",
            "session usage": "utilizzo sessione",
            "available": "disponibile",
            "Update": "Aggiorna",
            "Edit cards": "Modifica schede",
            "Done": "Fatto",
            "Updated {duration} ago": "Aggiornato {duration} fa",
            "Next update in {duration}": "Prossimo aggiornamento tra {duration}",
            "Other models aren't reported by the API yet": "Gli altri modelli non sono ancora riportati dall'API",
            "Quota Reset": "Quota ripristinata",
            "quota has been reset. Claude is ready to use again.": "la quota è stata ripristinata. Claude è pronto all'uso.",
            "Usage Notice": "Avviso di utilizzo",
            "usage has reached": "l'utilizzo ha raggiunto",
            "Notifications:": "Notifiche:",
            "Threshold and reset notifications": "Notifiche di soglia e ripristino",
            "Widget style:": "Stile widget:",
            "Sections": "Sezioni",
            "Scrollable content:": "Contenuto scorrevole:",
            "Enable scrollable content": "Abilita contenuto scorrevole",
            "Classic": "Classico",
            "Card": "Scheda",
            "Panel icon:": "Icona pannello:",
            "Ring": "Anello",
            "Update check": "Controllo aggiornamenti",
            "Check for Claude Code updates": "Verifica aggiornamenti Claude Code",
            "Installations": "Installazioni",
            "Show Claude Code installations": "Mostra installazioni Claude Code",
            "Auto-retry in": "Riprova automatica tra",
            "Colors:": "Colori:",
            "Theme": "Tema",
            "Time-proportional warnings": "Avvisi proporzionali al tempo",
            "Authentication": "Autenticazione",
            "Credentials path:": "Percorso credenziali:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": "Percorso completo di .credentials.json, o la cartella che lo contiene. La stessa cartella viene usata per le informazioni dell'account e le statistiche dei token. Lasciare vuoto per usare $CLAUDE_CONFIG_DIR, o ~/.claude se non è impostata.",
            "Session refresh:": "Aggiornamento sessione:",
            "Automatically refresh expired session": "Aggiorna automaticamente la sessione scaduta",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "Riapre brevemente claude in background (circa 15 secondi, senza finestra visibile, senza inviare messaggi) per aggiornare la sessione prima di chiederti di accedere manualmente."
        },
        "pt_BR": {
            "weekly usage": "uso semanal",
            "Colors": "Cores",
            "Token Stats (Today)": "Estatísticas de tokens (hoje)",
            "Claude Usage": "Uso do Claude",
            "Session (5hr)": "Sessão (5h)",
            "Weekly (7day)": "Semanal (7 dias)",
            "By Model (Weekly)": "Por modelo (Semanal)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "Reinicia às:",
            "Resets:": "Reinicia:",
            "Updated:": "Atualizado:",
            "Loading...": "Carregando...",
            "Refresh": "Atualizar",
            "Not logged in": "Não conectado",
            "Token expired": "Token expirado",
            "Re-login required": "Novo login necessário",
            "API error": "Erro de API",
            "Run 'claude' to log in": "Execute 'claude' para fazer login",
            "Will retry automatically": "Tentativa automática",
            "Network error - showing cached data": "Erro de rede - dados em cache",
            "No model breakdown available": "Sem detalhamento por modelo",
            "Language:": "Idioma:",
            "System default": "Padrão do sistema",
            "d": "d",
            "h": "h",
            "m": "min",
            "Refresh interval:": "Intervalo de atualização:",
            "minutes": "minutos",
            "Open Claude": "Abrir Claude",
            "Rate limited": "Limite atingido",
            "Auto-retry in 1 min": "Tentativa automática em 1 min",
            "Custom API (optional)": "API personalizada (opcional)",
            "Base URL:": "URL base:",
            "API key:": "Chave API:",
            "Leave empty to use the credentials file (default)": "Deixe vazio para usar o arquivo de credenciais (padrão)",
            "Check base URL and API key in widget settings": "Verifique a URL base e a chave API nas configurações do widget",
            "API key not configured": "Chave API não configurada",
            "Invalid API key": "Chave API inválida",
            "Endpoint not found": "Endpoint não encontrado",
            "Values under 5 min may cause rate limiting": "Valores abaixo de 5 min podem causar rate limiting",
            "Panel display": "Exibição do painel",
            "Show in panel:": "Mostrar no painel:",
            "Icon:": "Ícone:",
            "Icon size:": "Tamanho do ícone:",
            "Metrics size (panel and popup):": "Tamanho das métricas (painel e popup):",
            "Auto": "Automático",
            "Show Claude icon": "Mostrar ícone do Claude",
            "Style:": "Estilo:",
            "Text": "Texto",
            "Bar": "Barra",
            "Layout:": "Layout:",
            "Horizontal": "Horizontal",
            "Vertical": "Vertical",
            "Background opacity (desktop):": "Opacidade do fundo (área de trabalho):",
            "No model data yet": "Sem dados de modelo ainda",
            "Popup cards": "Cartões do popup",
            "Reset to default": "Restaurar padrão",
            "Session & Weekly": "Sessão e semanal",
            "Quick links": "Links rápidos",
            "Add link": "Adicionar link",
            "Show quick links in popup": "Mostrar links rápidos no popup",
            "Name": "Nome",
            "Visibility": "Visibilidade",
            "When Claude is not running:": "Quando o Claude não está em execução:",
            "Always show full widget": "Sempre mostrar widget completo",
            "Hide usage, only show icon": "Ocultar uso, mostrar apenas ícone",
            "Fully hide widget": "Ocultar completamente o widget",
            "Process check interval:": "Intervalo de verificação de processo:",
            "seconds": "segundos",
            "Account": "Conta",
            "Email": "E-mail",
            "Plan": "Plano",
            "resets in": "reinicia em",
            "Extra Usage": "Uso extra",
            "spent": "gasto",
            "Today's Tokens": "Tokens de hoje",
            "local logs": "logs locais",
            "out": "saída",
            "7-day trend": "Tendência 7 dias",
            "session usage": "uso da sessão",
            "available": "disponível",
            "Update": "Atualizar",
            "Edit cards": "Editar cartões",
            "Done": "Pronto",
            "Updated {duration} ago": "Atualizado há {duration}",
            "Next update in {duration}": "Próxima atualização em {duration}",
            "Other models aren't reported by the API yet": "Outros modelos ainda não são reportados pela API",
            "Quota Reset": "Cota redefinida",
            "quota has been reset. Claude is ready to use again.": "a cota foi redefinida. O Claude está pronto para usar.",
            "Usage Notice": "Aviso de uso",
            "usage has reached": "o uso atingiu",
            "Notifications:": "Notificações:",
            "Threshold and reset notifications": "Notificações de limite e redefinição",
            "Widget style:": "Estilo do widget:",
            "Sections": "Seções",
            "Scrollable content:": "Conteúdo rolável:",
            "Enable scrollable content": "Ativar conteúdo rolável",
            "Classic": "Clássico",
            "Card": "Cartão",
            "Panel icon:": "Ícone do painel:",
            "Ring": "Anel",
            "Update check": "Verificar atualizações",
            "Check for Claude Code updates": "Verificar atualizações do Claude Code",
            "Installations": "Instalações",
            "Show Claude Code installations": "Mostrar instalações do Claude Code",
            "Auto-retry in": "Tentativa automática em",
            "Colors:": "Cores:",
            "Theme": "Tema",
            "Time-proportional warnings": "Avisos proporcionais ao tempo",
            "Authentication": "Autenticação",
            "Credentials path:": "Caminho das credenciais:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": "Caminho completo para .credentials.json, ou a pasta que o contém. A mesma pasta é usada para as informações da conta e as estatísticas de tokens. Deixe vazio para usar $CLAUDE_CONFIG_DIR, ou ~/.claude se não estiver definida.",
            "Session refresh:": "Atualização de sessão:",
            "Automatically refresh expired session": "Atualizar automaticamente a sessão expirada",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "Reabre brevemente o claude em segundo plano (cerca de 15 segundos, sem janela visível, sem enviar mensagem) para atualizar a sessão antes de solicitar que você faça login manualmente."
        },
        "ru_RU": {
            "weekly usage": "недельное использование",
            "Colors": "Цвета",
            "Token Stats (Today)": "Статистика токенов (сегодня)",
            "Claude Usage": "Использование Claude",
            "Session (5hr)": "Сессия (5ч)",
            "Weekly (7day)": "Неделя (7 дней)",
            "By Model (Weekly)": "По модели (Неделя)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "Сброс в:",
            "Resets:": "Сброс:",
            "Updated:": "Обновлено:",
            "Loading...": "Загрузка...",
            "Refresh": "Обновить",
            "Not logged in": "Не авторизован",
            "Token expired": "Токен истёк",
            "Re-login required": "Требуется повторный вход",
            "API error": "Ошибка API",
            "Run 'claude' to log in": "Запустите 'claude' для входа",
            "Will retry automatically": "Автоматическая повторная попытка",
            "Network error - showing cached data": "Ошибка сети - показаны кэшированные данные",
            "No model breakdown available": "Нет данных по моделям",
            "Language:": "Язык:",
            "System default": "По умолчанию системы",
            "d": "д",
            "h": "ч",
            "m": "мин",
            "Refresh interval:": "Интервал обновления:",
            "minutes": "минут",
            "Open Claude": "Открыть Claude",
            "Rate limited": "Лимит достигнут",
            "Auto-retry in 1 min": "Автоповтор через 1 мин",
            "Custom API (optional)": "Пользовательский API (необязательно)",
            "Base URL:": "Базовый URL:",
            "API key:": "API ключ:",
            "Leave empty to use the credentials file (default)": "Оставьте пустым для использования файла учётных данных (по умолчанию)",
            "Check base URL and API key in widget settings": "Проверьте базовый URL и API ключ в настройках виджета",
            "API key not configured": "API ключ не настроен",
            "Invalid API key": "Неверный API ключ",
            "Endpoint not found": "Конечная точка не найдена",
            "Values under 5 min may cause rate limiting": "Значения менее 5 мин могут вызвать ограничение запросов",
            "Panel display": "Отображение панели",
            "Show in panel:": "Показывать в панели:",
            "Icon:": "Иконка:",
            "Icon size:": "Размер иконки:",
            "Metrics size (panel and popup):": "Размер метрик (панель и окно):",
            "Auto": "Авто",
            "Show Claude icon": "Показать иконку Claude",
            "Style:": "Стиль:",
            "Text": "Текст",
            "Bar": "Столбик",
            "Layout:": "Расположение:",
            "Horizontal": "Горизонтально",
            "Vertical": "Вертикально",
            "Background opacity (desktop):": "Прозрачность фона (рабочий стол):",
            "No model data yet": "Данные моделей ещё не получены",
            "Popup cards": "Карточки всплывающего окна",
            "Reset to default": "Сбросить по умолчанию",
            "Session & Weekly": "Сессия и неделя",
            "Quick links": "Быстрые ссылки",
            "Add link": "Добавить ссылку",
            "Show quick links in popup": "Показать быстрые ссылки во всплывающем окне",
            "Name": "Имя",
            "Visibility": "Видимость",
            "When Claude is not running:": "Когда Claude не запущен:",
            "Always show full widget": "Всегда показывать полный виджет",
            "Hide usage, only show icon": "Скрыть использование, показать только значок",
            "Fully hide widget": "Полностью скрыть виджет",
            "Process check interval:": "Интервал проверки процесса:",
            "seconds": "секунд",
            "Account": "Аккаунт",
            "Email": "Эл. почта",
            "Plan": "Тариф",
            "resets in": "сброс через",
            "Extra Usage": "Доп. использование",
            "spent": "потрачено",
            "Today's Tokens": "Токены за сегодня",
            "local logs": "локальные логи",
            "out": "выход",
            "7-day trend": "Тренд за 7 дней",
            "session usage": "использование сессии",
            "available": "доступно",
            "Update": "Обновить",
            "Edit cards": "Редактировать карточки",
            "Done": "Готово",
            "Updated {duration} ago": "Обновлено {duration} назад",
            "Next update in {duration}": "Следующее обновление через {duration}",
            "Other models aren't reported by the API yet": "Другие модели пока не отображаются в API",
            "Quota Reset": "Квота сброшена",
            "quota has been reset. Claude is ready to use again.": "квота сброшена. Claude снова готов к использованию.",
            "Usage Notice": "Уведомление об использовании",
            "usage has reached": "использование достигло",
            "Notifications:": "Уведомления:",
            "Threshold and reset notifications": "Уведомления о пороге и сбросе",
            "Widget style:": "Стиль виджета:",
            "Sections": "Разделы",
            "Scrollable content:": "Прокручиваемое содержимое:",
            "Enable scrollable content": "Включить прокручиваемое содержимое",
            "Classic": "Классический",
            "Card": "Карточка",
            "Panel icon:": "Значок панели:",
            "Ring": "Кольцо",
            "Update check": "Проверка обновлений",
            "Check for Claude Code updates": "Проверять обновления Claude Code",
            "Installations": "Установки",
            "Show Claude Code installations": "Показать установки Claude Code",
            "Auto-retry in": "Автоповтор через",
            "Colors:": "Цвета:",
            "Theme": "Тема",
            "Time-proportional warnings": "Пропорциональные времени предупреждения",
            "Authentication": "Аутентификация",
            "Credentials path:": "Путь к учётным данным:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": "Полный путь к .credentials.json или папка, в которой он находится. Эта же папка используется для данных аккаунта и статистики токенов. Оставьте пустым для использования $CLAUDE_CONFIG_DIR или ~/.claude, если она не задана.",
            "Session refresh:": "Обновление сессии:",
            "Automatically refresh expired session": "Автоматически обновлять истёкшую сессию",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "Ненадолго открывает claude в фоновом режиме (около 15 секунд, без видимого окна, без отправки сообщений), чтобы обновить сессию перед запросом на ручной вход."
        },
        "pl_PL": {
            "weekly usage": "tygodniowe użycie",
            "Colors": "Kolory",
            "Token Stats (Today)": "Statystyki tokenów (dzisiaj)",
            "Claude Usage": "Użycie Claude",
            "Session (5hr)": "Sesja (5h)",
            "Weekly (7day)": "Tygodniowo (7 dni)",
            "By Model (Weekly)": "Wg modelu (Tygodniowo)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "Resetuje o:",
            "Resets:": "Resetuje:",
            "Updated:": "Zaktualizowano:",
            "Loading...": "Ładowanie...",
            "Refresh": "Odśwież",
            "Not logged in": "Nie zalogowany",
            "Token expired": "Token wygasł",
            "Re-login required": "Wymagane ponowne logowanie",
            "API error": "Błąd API",
            "Run 'claude' to log in": "Uruchom 'claude' aby się zalogować",
            "Will retry automatically": "Automatyczne ponowienie",
            "Network error - showing cached data": "Błąd sieci - dane z pamięci podręcznej",
            "No model breakdown available": "Brak podziału na modele",
            "Language:": "Język:",
            "System default": "Domyślny systemowy",
            "d": "d",
            "h": "g",
            "m": "min",
            "Refresh interval:": "Częstotliwość odświeżania:",
            "minutes": "minut",
            "Open Claude": "Otwórz Claude",
            "Rate limited": "Limit osiągnięty",
            "Auto-retry in 1 min": "Automatyczne ponowienie za 1 min",
            "Custom API (optional)": "Własne API (opcjonalne)",
            "Base URL:": "Bazowy URL:",
            "API key:": "Klucz API:",
            "Leave empty to use the credentials file (default)": "Pozostaw puste, aby użyć pliku poświadczeń (domyślne)",
            "Check base URL and API key in widget settings": "Sprawdź bazowy URL i klucz API w ustawieniach widgetu",
            "API key not configured": "Klucz API nie skonfigurowany",
            "Invalid API key": "Nieprawidłowy klucz API",
            "Endpoint not found": "Punkt końcowy nie znaleziony",
            "Values under 5 min may cause rate limiting": "Wartości poniżej 5 min mogą powodować rate limiting",
            "Panel display": "Wyświetlanie panelu",
            "Show in panel:": "Pokaż w panelu:",
            "Icon:": "Ikona:",
            "Icon size:": "Rozmiar ikony:",
            "Metrics size (panel and popup):": "Rozmiar wskaźników (panel i okno):",
            "Auto": "Automatyczny",
            "Show Claude icon": "Pokaż ikonę Claude",
            "Style:": "Styl:",
            "Text": "Tekst",
            "Bar": "Pasek",
            "Layout:": "Układ:",
            "Horizontal": "Poziomy",
            "Vertical": "Pionowy",
            "Background opacity (desktop):": "Przezroczystość tła (pulpit):",
            "No model data yet": "Brak danych modeli",
            "Popup cards": "Karty wyskakującego okna",
            "Reset to default": "Przywróć domyślne",
            "Session & Weekly": "Sesja i tygodniowo",
            "Quick links": "Szybkie linki",
            "Add link": "Dodaj link",
            "Show quick links in popup": "Pokaż szybkie linki w okienku",
            "Name": "Nazwa",
            "Visibility": "Widoczność",
            "When Claude is not running:": "Gdy Claude nie jest uruchomiony:",
            "Always show full widget": "Zawsze pokazuj pełny widget",
            "Hide usage, only show icon": "Ukryj użycie, pokaż tylko ikonę",
            "Fully hide widget": "Całkowicie ukryj widget",
            "Process check interval:": "Interwał sprawdzania procesu:",
            "seconds": "sekund",
            "Account": "Konto",
            "Email": "E-mail",
            "Plan": "Plan",
            "resets in": "resetuje za",
            "Extra Usage": "Dodatkowe użycie",
            "spent": "wydane",
            "Today's Tokens": "Dzisiejsze tokeny",
            "local logs": "lokalne logi",
            "out": "wyjście",
            "7-day trend": "Trend 7-dniowy",
            "session usage": "użycie sesji",
            "available": "dostępne",
            "Update": "Aktualizuj",
            "Edit cards": "Edytuj karty",
            "Done": "Gotowe",
            "Updated {duration} ago": "Zaktualizowano {duration} temu",
            "Next update in {duration}": "Następna aktualizacja za {duration}",
            "Other models aren't reported by the API yet": "Inne modele nie są jeszcze raportowane przez API",
            "Quota Reset": "Limit zresetowany",
            "quota has been reset. Claude is ready to use again.": "limit został zresetowany. Claude jest ponownie gotowy.",
            "Usage Notice": "Powiadomienie o użyciu",
            "usage has reached": "użycie osiągnęło",
            "Notifications:": "Powiadomienia:",
            "Threshold and reset notifications": "Powiadomienia o progach i resetach",
            "Widget style:": "Styl widżetu:",
            "Sections": "Sekcje",
            "Scrollable content:": "Przewijana treść:",
            "Enable scrollable content": "Włącz przewijaną treść",
            "Classic": "Klasyczny",
            "Card": "Karta",
            "Panel icon:": "Ikona panelu:",
            "Ring": "Pierścień",
            "Update check": "Sprawdzanie aktualizacji",
            "Check for Claude Code updates": "Sprawdzaj aktualizacje Claude Code",
            "Installations": "Instalacje",
            "Show Claude Code installations": "Pokaż instalacje Claude Code",
            "Auto-retry in": "Automatyczne ponowienie za",
            "Colors:": "Kolory:",
            "Theme": "Motyw",
            "Time-proportional warnings": "Ostrzeżenia proporcjonalne do czasu",
            "Authentication": "Uwierzytelnianie",
            "Credentials path:": "Ścieżka do poświadczeń:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": "Pełna ścieżka do .credentials.json lub folder, który go zawiera. Ten sam folder służy do informacji o koncie i statystyk tokenów. Pozostaw puste, aby użyć $CLAUDE_CONFIG_DIR lub ~/.claude, jeśli nie jest ustawiona.",
            "Session refresh:": "Odświeżanie sesji:",
            "Automatically refresh expired session": "Automatycznie odświeżaj wygasłą sesję",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "Na krótko ponownie otwiera claude w tle (około 15 sekund, bez widocznego okna, bez wysyłania wiadomości), aby odświeżyć sesję przed poproszeniem o ręczne zalogowanie."
        },
        "nl_NL": {
            "weekly usage": "wekelijks gebruik",
            "Colors": "Kleuren",
            "Token Stats (Today)": "Tokenstatistieken (vandaag)",
            "Claude Usage": "Claude Gebruik",
            "Session (5hr)": "Sessie (5u)",
            "Weekly (7day)": "Wekelijks (7 dagen)",
            "By Model (Weekly)": "Per model (Wekelijks)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "Reset om:",
            "Resets:": "Reset:",
            "Updated:": "Bijgewerkt:",
            "Loading...": "Laden...",
            "Refresh": "Vernieuwen",
            "Not logged in": "Niet ingelogd",
            "Token expired": "Token verlopen",
            "Re-login required": "Opnieuw inloggen vereist",
            "API error": "API-fout",
            "Run 'claude' to log in": "Voer 'claude' uit om in te loggen",
            "Will retry automatically": "Automatisch opnieuw proberen",
            "Network error - showing cached data": "Netwerkfout - gecachte gegevens",
            "No model breakdown available": "Geen modeluitsplitsing beschikbaar",
            "Language:": "Taal:",
            "System default": "Systeemstandaard",
            "d": "d",
            "h": "u",
            "m": "min",
            "Refresh interval:": "Vernieuwingsinterval:",
            "minutes": "minuten",
            "Open Claude": "Claude openen",
            "Rate limited": "Limiet bereikt",
            "Auto-retry in 1 min": "Automatisch opnieuw over 1 min",
            "Custom API (optional)": "Aangepaste API (optioneel)",
            "Base URL:": "Basis-URL:",
            "API key:": "API-sleutel:",
            "Leave empty to use the credentials file (default)": "Laat leeg om het bestand met inloggegevens te gebruiken (standaard)",
            "Check base URL and API key in widget settings": "Controleer basis-URL en API-sleutel in de widgetinstellingen",
            "API key not configured": "API-sleutel niet geconfigureerd",
            "Invalid API key": "Ongeldige API-sleutel",
            "Endpoint not found": "Eindpunt niet gevonden",
            "Values under 5 min may cause rate limiting": "Waarden onder 5 min kunnen rate limiting veroorzaken",
            "Panel display": "Panelweergave",
            "Show in panel:": "Tonen in paneel:",
            "Icon:": "Pictogram:",
            "Icon size:": "Pictogramgrootte:",
            "Metrics size (panel and popup):": "Metriekgrootte (paneel en pop-up):",
            "Auto": "Automatisch",
            "Show Claude icon": "Claude-pictogram tonen",
            "Style:": "Stijl:",
            "Text": "Tekst",
            "Bar": "Balk",
            "Layout:": "Indeling:",
            "Horizontal": "Horizontaal",
            "Vertical": "Verticaal",
            "Background opacity (desktop):": "Achtergrond dekking (bureaublad):",
            "No model data yet": "Nog geen modelgegevens",
            "Popup cards": "Popup-kaarten",
            "Reset to default": "Standaard herstellen",
            "Session & Weekly": "Sessie & wekelijks",
            "Quick links": "Snelkoppelingen",
            "Add link": "Link toevoegen",
            "Show quick links in popup": "Snelkoppelingen in popup tonen",
            "Name": "Naam",
            "Visibility": "Zichtbaarheid",
            "When Claude is not running:": "Wanneer Claude niet draait:",
            "Always show full widget": "Widget altijd volledig tonen",
            "Hide usage, only show icon": "Gebruik verbergen, alleen pictogram tonen",
            "Fully hide widget": "Widget volledig verbergen",
            "Process check interval:": "Procescontrole-interval:",
            "seconds": "seconden",
            "Account": "Account",
            "Email": "E-mail",
            "Plan": "Abonnement",
            "resets in": "reset over",
            "Extra Usage": "Extra gebruik",
            "spent": "besteed",
            "Today's Tokens": "Tokens van vandaag",
            "local logs": "lokale logs",
            "out": "uitvoer",
            "7-day trend": "7-dagentrend",
            "session usage": "sessiegebruik",
            "available": "beschikbaar",
            "Update": "Bijwerken",
            "Edit cards": "Kaarten bewerken",
            "Done": "Klaar",
            "Updated {duration} ago": "{duration} geleden bijgewerkt",
            "Next update in {duration}": "Volgende update over {duration}",
            "Other models aren't reported by the API yet": "Andere modellen worden nog niet door de API gemeld",
            "Quota Reset": "Quotum gereset",
            "quota has been reset. Claude is ready to use again.": "quotum is gereset. Claude is weer klaar voor gebruik.",
            "Usage Notice": "Gebruiksmelding",
            "usage has reached": "gebruik heeft bereikt",
            "Notifications:": "Meldingen:",
            "Threshold and reset notifications": "Drempelwaarde- en resetmeldingen",
            "Widget style:": "Widgetstijl:",
            "Sections": "Secties",
            "Scrollable content:": "Scrollbare inhoud:",
            "Enable scrollable content": "Scrollbare inhoud inschakelen",
            "Classic": "Klassiek",
            "Card": "Kaart",
            "Panel icon:": "Paneelpictogram:",
            "Ring": "Ring",
            "Update check": "Updatecontrole",
            "Check for Claude Code updates": "Controleren op Claude Code updates",
            "Installations": "Installaties",
            "Show Claude Code installations": "Claude Code installaties tonen",
            "Auto-retry in": "Automatisch opnieuw over",
            "Colors:": "Kleuren:",
            "Theme": "Thema",
            "Time-proportional warnings": "Tijdsproportionele waarschuwingen",
            "Authentication": "Authenticatie",
            "Credentials path:": "Pad naar inloggegevens:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": "Volledig pad naar .credentials.json, of de map die het bevat. Dezelfde map wordt gebruikt voor accountgegevens en tokenstatistieken. Laat leeg om $CLAUDE_CONFIG_DIR te gebruiken, of ~/.claude als die niet is ingesteld.",
            "Session refresh:": "Sessievernieuwing:",
            "Automatically refresh expired session": "Verlopen sessie automatisch vernieuwen",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "Heropent claude kort op de achtergrond (ongeveer 15 seconden, geen zichtbaar venster, geen bericht verzonden) om de sessie te vernieuwen voordat u wordt gevraagd handmatig in te loggen."
        },
        "tr_TR": {
            "weekly usage": "haftalık kullanım",
            "Colors": "Renkler",
            "Token Stats (Today)": "Jeton istatistikleri (bugün)",
            "Claude Usage": "Claude Kullanımı",
            "Session (5hr)": "Oturum (5sa)",
            "Weekly (7day)": "Haftalık (7 gün)",
            "By Model (Weekly)": "Modele göre (Haftalık)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "Sıfırlanma:",
            "Resets:": "Sıfırlanma:",
            "Updated:": "Güncellendi:",
            "Loading...": "Yükleniyor...",
            "Refresh": "Yenile",
            "Not logged in": "Giriş yapılmadı",
            "Token expired": "Token süresi doldu",
            "Re-login required": "Yeniden giriş gerekli",
            "API error": "API hatası",
            "Run 'claude' to log in": "Giriş için 'claude' çalıştırın",
            "Will retry automatically": "Otomatik yeniden deneme",
            "Network error - showing cached data": "Ağ hatası - önbellek verileri gösteriliyor",
            "No model breakdown available": "Model dağılımı mevcut değil",
            "Language:": "Dil:",
            "System default": "Sistem varsayılanı",
            "d": "g",
            "h": "sa",
            "m": "dk",
            "Refresh interval:": "Yenileme aralığı:",
            "minutes": "dakika",
            "Open Claude": "Claude'u Aç",
            "Rate limited": "Limit aşıldı",
            "Auto-retry in 1 min": "1 dk sonra otomatik yeniden deneme",
            "Custom API (optional)": "Özel API (isteğe bağlı)",
            "Base URL:": "Temel URL:",
            "API key:": "API anahtarı:",
            "Leave empty to use the credentials file (default)": "Kimlik bilgileri dosyasını kullanmak için boş bırakın (varsayılan)",
            "Check base URL and API key in widget settings": "Widget ayarlarında temel URL ve API anahtarını kontrol edin",
            "API key not configured": "API anahtarı yapılandırılmadı",
            "Invalid API key": "Geçersiz API anahtarı",
            "Endpoint not found": "Uç nokta bulunamadı",
            "Values under 5 min may cause rate limiting": "5 dk altındaki değerler rate limiting'e neden olabilir",
            "Panel display": "Panel görünümü",
            "Show in panel:": "Panelde göster:",
            "Icon:": "Simge:",
            "Icon size:": "Simge boyutu:",
            "Metrics size (panel and popup):": "Ölçüm boyutu (panel ve açılır pencere):",
            "Auto": "Otomatik",
            "Show Claude icon": "Claude simgesini göster",
            "Style:": "Stil:",
            "Text": "Metin",
            "Bar": "Çubuk",
            "Layout:": "Düzen:",
            "Horizontal": "Yatay",
            "Vertical": "Dikey",
            "Background opacity (desktop):": "Arka plan opaklığı (masaüstü):",
            "No model data yet": "Henüz model verisi yok",
            "Popup cards": "Açılır kartlar",
            "Reset to default": "Varsayılana sıfırla",
            "Session & Weekly": "Oturum ve haftalık",
            "Quick links": "Hızlı bağlantılar",
            "Add link": "Bağlantı ekle",
            "Show quick links in popup": "Açılır pencerede hızlı bağlantıları göster",
            "Name": "Ad",
            "Visibility": "Görünürlük",
            "When Claude is not running:": "Claude çalışmıyorken:",
            "Always show full widget": "Widget'ı her zaman tam göster",
            "Hide usage, only show icon": "Kullanımı gizle, sadece simgeyi göster",
            "Fully hide widget": "Widget'ı tamamen gizle",
            "Process check interval:": "İşlem kontrol aralığı:",
            "seconds": "saniye",
            "Account": "Hesap",
            "Email": "E-posta",
            "Plan": "Plan",
            "resets in": "sıfırlanma",
            "Extra Usage": "Ek kullanım",
            "spent": "harcanan",
            "Today's Tokens": "Bugünün tokenleri",
            "local logs": "yerel günlükler",
            "out": "çıkış",
            "7-day trend": "7 günlük eğilim",
            "session usage": "oturum kullanımı",
            "available": "mevcut",
            "Update": "Güncelle",
            "Edit cards": "Kartları düzenle",
            "Done": "Tamam",
            "Updated {duration} ago": "{duration} önce güncellendi",
            "Next update in {duration}": "Sonraki güncelleme {duration} sonra",
            "Other models aren't reported by the API yet": "Diğer modeller henüz API tarafından raporlanmıyor",
            "Quota Reset": "Kota sıfırlandı",
            "quota has been reset. Claude is ready to use again.": "kota sıfırlandı. Claude tekrar kullanıma hazır.",
            "Usage Notice": "Kullanım bildirimi",
            "usage has reached": "kullanım şuna ulaştı:",
            "Notifications:": "Bildirimler:",
            "Threshold and reset notifications": "Eşik ve sıfırlama bildirimleri",
            "Widget style:": "Widget stili:",
            "Sections": "Bölümler",
            "Scrollable content:": "Kaydırılabilir içerik:",
            "Enable scrollable content": "Kaydırılabilir içeriği etkinleştir",
            "Classic": "Klasik",
            "Card": "Kart",
            "Panel icon:": "Panel simgesi:",
            "Ring": "Halka",
            "Update check": "Güncelleme kontrolü",
            "Check for Claude Code updates": "Claude Code güncellemelerini kontrol et",
            "Installations": "Kurulumlar",
            "Show Claude Code installations": "Claude Code kurulumlarını göster",
            "Auto-retry in": "Otomatik yeniden deneme",
            "Colors:": "Renkler:",
            "Theme": "Tema",
            "Time-proportional warnings": "Zamana orantılı uyarılar",
            "Authentication": "Kimlik doğrulama",
            "Credentials path:": "Kimlik bilgileri yolu:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": ".credentials.json dosyasının tam yolu veya onu içeren klasör. Aynı klasör hesap bilgileri ve token istatistikleri için de kullanılır. $CLAUDE_CONFIG_DIR kullanmak için boş bırakın; ayarlı değilse ~/.claude kullanılır.",
            "Session refresh:": "Oturum yenileme:",
            "Automatically refresh expired session": "Süresi dolan oturumu otomatik yenile",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "Oturumu yenilemek için claude'u arka planda kısaca yeniden açar (yaklaşık 15 saniye, görünür pencere yok, mesaj gönderilmez), ardından sizden manuel giriş yapmanızı ister."
        },
        "ja_JP": {
            "weekly usage": "週間使用量",
            "Colors": "色",
            "Token Stats (Today)": "トークン統計（今日）",
            "Claude Usage": "Claude 使用状況",
            "Session (5hr)": "セッション (5時間)",
            "Weekly (7day)": "週間 (7日)",
            "By Model (Weekly)": "モデル別 (週間)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "リセット:",
            "Resets:": "リセット:",
            "Updated:": "更新:",
            "Loading...": "読み込み中...",
            "Refresh": "更新",
            "Not logged in": "未ログイン",
            "Token expired": "トークン期限切れ",
            "Re-login required": "再ログインが必要",
            "API error": "APIエラー",
            "Run 'claude' to log in": "'claude'を実行してログイン",
            "Will retry automatically": "自動的に再試行します",
            "Network error - showing cached data": "ネットワークエラー - キャッシュデータを表示中",
            "No model breakdown available": "モデル別データなし",
            "Language:": "言語:",
            "System default": "システムデフォルト",
            "d": "日",
            "h": "時",
            "m": "分",
            "Refresh interval:": "更新間隔:",
            "minutes": "分",
            "Open Claude": "Claudeを開く",
            "Rate limited": "レート制限",
            "Auto-retry in 1 min": "1分後に自動リトライ",
            "Custom API (optional)": "カスタムAPI（オプション）",
            "Base URL:": "ベースURL:",
            "API key:": "APIキー:",
            "Leave empty to use the credentials file (default)": "空欄で認証情報ファイルを使用（デフォルト）",
            "Check base URL and API key in widget settings": "ウィジェット設定でベースURLとAPIキーを確認",
            "API key not configured": "APIキー未設定",
            "Invalid API key": "無効なAPIキー",
            "Endpoint not found": "エンドポイントが見つかりません",
            "Values under 5 min may cause rate limiting": "5分未満の値はレート制限を引き起こす可能性があります",
            "Panel display": "パネル表示",
            "Show in panel:": "パネルに表示:",
            "Icon:": "アイコン:",
            "Icon size:": "アイコンサイズ:",
            "Metrics size (panel and popup):": "メトリクスサイズ (パネルとポップアップ):",
            "Auto": "自動",
            "Show Claude icon": "Claudeアイコンを表示",
            "Style:": "スタイル:",
            "Text": "テキスト",
            "Bar": "バー",
            "Layout:": "レイアウト:",
            "Horizontal": "横",
            "Vertical": "縦",
            "Background opacity (desktop):": "背景の不透明度（デスクトップ）:",
            "No model data yet": "モデルデータなし",
            "Popup cards": "ポップアップカード",
            "Reset to default": "デフォルトに戻す",
            "Session & Weekly": "セッション＆週間",
            "Quick links": "クイックリンク",
            "Add link": "リンクを追加",
            "Show quick links in popup": "ポップアップにクイックリンクを表示",
            "Name": "名前",
            "Visibility": "表示設定",
            "When Claude is not running:": "Claudeが実行されていない時:",
            "Always show full widget": "常にウィジェット全体を表示",
            "Hide usage, only show icon": "使用状況を隠し、アイコンのみ表示",
            "Fully hide widget": "ウィジェットを完全に非表示",
            "Process check interval:": "プロセスチェック間隔:",
            "seconds": "秒",
            "Account": "アカウント",
            "Email": "メール",
            "Plan": "プラン",
            "resets in": "リセットまで",
            "Extra Usage": "追加使用量",
            "spent": "使用済み",
            "Today's Tokens": "今日のトークン",
            "local logs": "ローカルログ",
            "out": "出力",
            "7-day trend": "7日間トレンド",
            "session usage": "セッション使用量",
            "available": "利用可能",
            "Update": "更新",
            "Edit cards": "カード編集",
            "Done": "完了",
            "Updated {duration} ago": "{duration}前に更新",
            "Next update in {duration}": "次の更新まで{duration}",
            "Other models aren't reported by the API yet": "他のモデルはまだAPIで報告されていません",
            "Quota Reset": "クォータリセット",
            "quota has been reset. Claude is ready to use again.": "クォータがリセットされました。Claudeを再び利用できます。",
            "Usage Notice": "使用量通知",
            "usage has reached": "使用量が到達",
            "Notifications:": "通知:",
            "Threshold and reset notifications": "しきい値とリセットの通知",
            "Widget style:": "ウィジェットスタイル:",
            "Sections": "セクション",
            "Scrollable content:": "スクロール可能なコンテンツ:",
            "Enable scrollable content": "スクロール可能なコンテンツを有効にする",
            "Classic": "クラシック",
            "Card": "カード",
            "Panel icon:": "パネルアイコン:",
            "Ring": "リング",
            "Update check": "更新チェック",
            "Check for Claude Code updates": "Claude Codeの更新を確認",
            "Installations": "インストール",
            "Show Claude Code installations": "Claude Codeのインストールを表示",
            "Auto-retry in": "自動リトライ",
            "Colors:": "色:",
            "Theme": "テーマ",
            "Time-proportional warnings": "時間比例の警告",
            "Authentication": "認証",
            "Credentials path:": "認証情報のパス:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": ".credentials.json の完全なパス、またはそれを含むフォルダー。同じフォルダーはアカウント情報とトークン統計にも使用されます。空欄の場合は $CLAUDE_CONFIG_DIR、未設定なら ~/.claude を使用します。",
            "Session refresh:": "セッション更新:",
            "Automatically refresh expired session": "期限切れのセッションを自動更新",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "手動ログインを求める前に、バックグラウンドで claude を短時間再起動し（約15秒、ウィンドウ表示なし、メッセージ送信なし）、セッションを更新します。"
        },
        "ko_KR": {
            "weekly usage": "주간 사용량",
            "Colors": "색상",
            "Token Stats (Today)": "토큰 통계 (오늘)",
            "Claude Usage": "Claude 사용량",
            "Session (5hr)": "세션 (5시간)",
            "Weekly (7day)": "주간 (7일)",
            "By Model (Weekly)": "모델별 (주간)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "초기화:",
            "Resets:": "초기화:",
            "Updated:": "업데이트:",
            "Loading...": "로딩 중...",
            "Refresh": "새로고침",
            "Not logged in": "로그인 안됨",
            "Token expired": "토큰 만료",
            "Re-login required": "재로그인 필요",
            "API error": "API 오류",
            "Run 'claude' to log in": "'claude'를 실행하여 로그인",
            "Will retry automatically": "자동 재시도",
            "Network error - showing cached data": "네트워크 오류 - 캐시된 데이터 표시 중",
            "No model breakdown available": "모델별 데이터 없음",
            "Language:": "언어:",
            "System default": "시스템 기본값",
            "d": "일",
            "h": "시",
            "m": "분",
            "Refresh interval:": "새로고침 간격:",
            "minutes": "분",
            "Open Claude": "Claude 열기",
            "Rate limited": "속도 제한",
            "Auto-retry in 1 min": "1분 후 자동 재시도",
            "Custom API (optional)": "사용자 정의 API (선택사항)",
            "Base URL:": "기본 URL:",
            "API key:": "API 키:",
            "Leave empty to use the credentials file (default)": "인증 정보 파일 사용 시 비워두세요 (기본값)",
            "Check base URL and API key in widget settings": "위젯 설정에서 기본 URL과 API 키를 확인하세요",
            "API key not configured": "API 키 미설정",
            "Invalid API key": "잘못된 API 키",
            "Endpoint not found": "엔드포인트를 찾을 수 없음",
            "Values under 5 min may cause rate limiting": "5분 미만의 값은 속도 제한을 유발할 수 있습니다",
            "Panel display": "패널 표시",
            "Show in panel:": "패널에 표시:",
            "Icon:": "아이콘:",
            "Icon size:": "아이콘 크기:",
            "Metrics size (panel and popup):": "지표 크기 (패널 및 팝업):",
            "Auto": "자동",
            "Show Claude icon": "Claude 아이콘 표시",
            "Style:": "스타일:",
            "Text": "텍스트",
            "Bar": "바",
            "Layout:": "레이아웃:",
            "Horizontal": "가로",
            "Vertical": "세로",
            "Background opacity (desktop):": "배경 불투명도 (데스크톱):",
            "No model data yet": "아직 모델 데이터 없음",
            "Popup cards": "팝업 카드",
            "Reset to default": "기본값으로 재설정",
            "Session & Weekly": "세션 & 주간",
            "Quick links": "빠른 링크",
            "Add link": "링크 추가",
            "Show quick links in popup": "팝업에 빠른 링크 표시",
            "Name": "이름",
            "Visibility": "가시성",
            "When Claude is not running:": "Claude가 실행되지 않을 때:",
            "Always show full widget": "항상 전체 위젯 표시",
            "Hide usage, only show icon": "사용량 숨기기, 아이콘만 표시",
            "Fully hide widget": "위젯 완전히 숨기기",
            "Process check interval:": "프로세스 확인 간격:",
            "seconds": "초",
            "Account": "계정",
            "Email": "이메일",
            "Plan": "요금제",
            "resets in": "초기화까지",
            "Extra Usage": "추가 사용량",
            "spent": "사용됨",
            "Today's Tokens": "오늘의 토큰",
            "local logs": "로컬 로그",
            "out": "출력",
            "7-day trend": "7일 추세",
            "session usage": "세션 사용량",
            "available": "사용 가능",
            "Update": "업데이트",
            "Edit cards": "카드 편집",
            "Done": "완료",
            "Updated {duration} ago": "{duration} 전에 업데이트됨",
            "Next update in {duration}": "다음 업데이트 {duration} 후",
            "Other models aren't reported by the API yet": "다른 모델은 아직 API에서 보고되지 않습니다",
            "Quota Reset": "할당량 초기화",
            "quota has been reset. Claude is ready to use again.": "할당량이 초기화되었습니다. Claude를 다시 사용할 수 있습니다.",
            "Usage Notice": "사용량 알림",
            "usage has reached": "사용량이 도달",
            "Notifications:": "알림:",
            "Threshold and reset notifications": "임계값 및 초기화 알림",
            "Widget style:": "위젯 스타일:",
            "Sections": "섹션",
            "Scrollable content:": "스크롤 가능한 콘텐츠:",
            "Enable scrollable content": "스크롤 가능한 콘텐츠 활성화",
            "Classic": "클래식",
            "Card": "카드",
            "Panel icon:": "패널 아이콘:",
            "Ring": "링",
            "Update check": "업데이트 확인",
            "Check for Claude Code updates": "Claude Code 업데이트 확인",
            "Installations": "설치",
            "Show Claude Code installations": "Claude Code 설치 표시",
            "Auto-retry in": "자동 재시도",
            "Colors:": "색상:",
            "Theme": "테마",
            "Time-proportional warnings": "시간 비례 경고",
            "Authentication": "인증",
            "Credentials path:": "인증 정보 경로:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": ".credentials.json의 전체 경로 또는 해당 파일이 있는 폴더. 같은 폴더가 계정 정보와 토큰 통계에도 사용됩니다. 비워두면 $CLAUDE_CONFIG_DIR을, 설정되지 않은 경우 ~/.claude를 사용합니다.",
            "Session refresh:": "세션 새로고침:",
            "Automatically refresh expired session": "만료된 세션 자동 새로고침",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "수동 로그인을 요청하기 전에 백그라운드에서 claude를 잠시 다시 열어(약 15초, 표시되는 창 없음, 메시지 전송 없음) 세션을 새로고침합니다."
        },
        "zh_CN": {
            "weekly usage": "每周使用量",
            "Colors": "颜色",
            "Token Stats (Today)": "令牌统计（今日）",
            "Claude Usage": "Claude 使用情况",
            "Session (5hr)": "会话 (5小时)",
            "Weekly (7day)": "每周 (7天)",
            "By Model (Weekly)": "按模型 (每周)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "重置时间:",
            "Resets:": "重置:",
            "Updated:": "更新时间:",
            "Loading...": "加载中...",
            "Refresh": "刷新",
            "Not logged in": "未登录",
            "Token expired": "令牌已过期",
            "Re-login required": "需要重新登录",
            "API error": "API 错误",
            "Run 'claude' to log in": "运行 'claude' 登录",
            "Will retry automatically": "将自动重试",
            "Network error - showing cached data": "网络错误 - 显示缓存数据",
            "No model breakdown available": "无模型明细",
            "Language:": "语言:",
            "System default": "系统默认",
            "d": "天",
            "h": "时",
            "m": "分",
            "Refresh interval:": "刷新间隔:",
            "minutes": "分钟",
            "Open Claude": "打开 Claude",
            "Rate limited": "速率限制",
            "Auto-retry in 1 min": "1分钟后自动重试",
            "Custom API (optional)": "自定义API（可选）",
            "Base URL:": "基础URL:",
            "API key:": "API密钥:",
            "Leave empty to use the credentials file (default)": "留空以使用凭据文件（默认）",
            "Check base URL and API key in widget settings": "请在小部件设置中检查基础URL和API密钥",
            "API key not configured": "API密钥未配置",
            "Invalid API key": "无效的API密钥",
            "Endpoint not found": "端点未找到",
            "Values under 5 min may cause rate limiting": "低于5分钟的值可能导致速率限制",
            "Panel display": "面板显示",
            "Show in panel:": "在面板中显示:",
            "Icon:": "图标:",
            "Icon size:": "图标大小:",
            "Metrics size (panel and popup):": "指标大小 (面板和弹窗):",
            "Auto": "自动",
            "Show Claude icon": "显示Claude图标",
            "Style:": "样式:",
            "Text": "文本",
            "Bar": "柱状",
            "Layout:": "布局:",
            "Horizontal": "水平",
            "Vertical": "垂直",
            "Background opacity (desktop):": "背景不透明度（桌面）:",
            "No model data yet": "暂无模型数据",
            "Popup cards": "弹出卡片",
            "Reset to default": "恢复默认",
            "Session & Weekly": "会话和每周",
            "Quick links": "快捷链接",
            "Add link": "添加链接",
            "Show quick links in popup": "在弹出窗口中显示快捷链接",
            "Name": "名称",
            "Visibility": "可见性",
            "When Claude is not running:": "当Claude未运行时:",
            "Always show full widget": "始终显示完整小部件",
            "Hide usage, only show icon": "隐藏使用情况，仅显示图标",
            "Fully hide widget": "完全隐藏小部件",
            "Process check interval:": "进程检查间隔:",
            "seconds": "秒",
            "Account": "账户",
            "Email": "邮箱",
            "Plan": "套餐",
            "resets in": "重置剩余",
            "Extra Usage": "额外用量",
            "spent": "已花费",
            "Today's Tokens": "今日Token",
            "local logs": "本地日志",
            "out": "输出",
            "7-day trend": "7天趋势",
            "session usage": "会话用量",
            "available": "可用",
            "Update": "更新",
            "Edit cards": "编辑卡片",
            "Done": "完成",
            "Updated {duration} ago": "{duration}前更新",
            "Next update in {duration}": "下次更新在{duration}后",
            "Other models aren't reported by the API yet": "其他模型暂未被API报告",
            "Quota Reset": "配额已重置",
            "quota has been reset. Claude is ready to use again.": "配额已重置。Claude可以再次使用了。",
            "Usage Notice": "使用量通知",
            "usage has reached": "使用量已达到",
            "Notifications:": "通知:",
            "Threshold and reset notifications": "阈值和重置通知",
            "Widget style:": "小部件样式:",
            "Sections": "部分",
            "Scrollable content:": "可滚动内容：",
            "Enable scrollable content": "启用可滚动内容",
            "Classic": "经典",
            "Card": "卡片",
            "Panel icon:": "面板图标:",
            "Ring": "环形",
            "Update check": "更新检查",
            "Check for Claude Code updates": "检查Claude Code更新",
            "Installations": "安装",
            "Show Claude Code installations": "显示Claude Code安装",
            "Auto-retry in": "自动重试",
            "Colors:": "颜色:",
            "Theme": "主题",
            "Time-proportional warnings": "时间比例警告",
            "Authentication": "身份验证",
            "Credentials path:": "凭据路径:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": ".credentials.json 的完整路径，或包含它的文件夹。同一文件夹也用于账户信息和 Token 统计。留空则使用 $CLAUDE_CONFIG_DIR，未设置时使用 ~/.claude。",
            "Session refresh:": "会话刷新:",
            "Automatically refresh expired session": "自动刷新过期会话",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "在提示您手动登录之前，在后台短暂重新打开 claude（约15秒，无可见窗口，不发送消息）以刷新会话。"
        },
        "zh_TW": {
            "weekly usage": "每週使用量",
            "Colors": "顏色",
            "Token Stats (Today)": "權杖統計（今日）",
            "Claude Usage": "Claude 使用狀況",
            "Session (5hr)": "工作階段 (5小時)",
            "Weekly (7day)": "每週 (7天)",
            "By Model (Weekly)": "依模型 (每週)",
            "Sonnet": "Sonnet",
            "Opus": "Opus",
            "Resets at:": "重置時間:",
            "Resets:": "重置:",
            "Updated:": "更新時間:",
            "Loading...": "載入中...",
            "Refresh": "重新整理",
            "Not logged in": "未登入",
            "Token expired": "權杖已過期",
            "Re-login required": "需要重新登入",
            "API error": "API 錯誤",
            "Run 'claude' to log in": "執行 'claude' 登入",
            "Will retry automatically": "將自動重試",
            "Network error - showing cached data": "網路錯誤 - 顯示快取資料",
            "No model breakdown available": "無模型明細",
            "Language:": "語言:",
            "System default": "系統預設",
            "d": "天",
            "h": "時",
            "m": "分",
            "Refresh interval:": "重新整理間隔:",
            "minutes": "分鐘",
            "Open Claude": "開啟 Claude",
            "Rate limited": "速率限制",
            "Auto-retry in 1 min": "1分鐘後自動重試",
            "Custom API (optional)": "自訂API（選填）",
            "Base URL:": "基礎URL:",
            "API key:": "API金鑰:",
            "Leave empty to use the credentials file (default)": "留空以使用憑證檔案（預設）",
            "Check base URL and API key in widget settings": "請在小工具設定中檢查基礎URL和API金鑰",
            "API key not configured": "API金鑰未設定",
            "Invalid API key": "無效的API金鑰",
            "Endpoint not found": "端點未找到",
            "Values under 5 min may cause rate limiting": "低於5分鐘的值可能導致速率限制",
            "Panel display": "面板顯示",
            "Show in panel:": "在面板中顯示:",
            "Icon:": "圖標:",
            "Icon size:": "圖標大小:",
            "Metrics size (panel and popup):": "指標大小 (面板和彈出視窗):",
            "Auto": "自動",
            "Show Claude icon": "顯示Claude圖標",
            "Style:": "樣式:",
            "Text": "文字",
            "Bar": "柱狀",
            "Layout:": "佈局:",
            "Horizontal": "水平",
            "Vertical": "垂直",
            "Background opacity (desktop):": "背景不透明度（桌面）:",
            "No model data yet": "尚無模型資料",
            "Popup cards": "彈出卡片",
            "Reset to default": "恢復預設",
            "Session & Weekly": "工作階段與每週",
            "Quick links": "快速連結",
            "Add link": "新增連結",
            "Show quick links in popup": "在彈出視窗中顯示快速連結",
            "Name": "名稱",
            "Visibility": "可見性",
            "When Claude is not running:": "當Claude未執行時:",
            "Always show full widget": "始終顯示完整小工具",
            "Hide usage, only show icon": "隱藏使用狀況，僅顯示圖標",
            "Fully hide widget": "完全隱藏小工具",
            "Process check interval:": "程序檢查間隔:",
            "seconds": "秒",
            "Account": "帳戶",
            "Email": "電子郵件",
            "Plan": "方案",
            "resets in": "重置剩餘",
            "Extra Usage": "額外用量",
            "spent": "已花費",
            "Today's Tokens": "今日Token",
            "local logs": "本機日誌",
            "out": "輸出",
            "7-day trend": "7天趨勢",
            "session usage": "工作階段用量",
            "available": "可用",
            "Update": "更新",
            "Edit cards": "編輯卡片",
            "Done": "完成",
            "Updated {duration} ago": "{duration}前更新",
            "Next update in {duration}": "下次更新在{duration}後",
            "Other models aren't reported by the API yet": "其他模型尚未被API回報",
            "Quota Reset": "配額已重置",
            "quota has been reset. Claude is ready to use again.": "配額已重置。Claude可以再次使用了。",
            "Usage Notice": "使用量通知",
            "usage has reached": "使用量已達到",
            "Notifications:": "通知:",
            "Threshold and reset notifications": "閾值和重置通知",
            "Widget style:": "小工具樣式:",
            "Sections": "區段",
            "Scrollable content:": "可捲動內容：",
            "Enable scrollable content": "啟用可捲動內容",
            "Classic": "經典",
            "Card": "卡片",
            "Panel icon:": "面板圖標:",
            "Ring": "環形",
            "Update check": "更新檢查",
            "Check for Claude Code updates": "檢查Claude Code更新",
            "Installations": "安裝",
            "Show Claude Code installations": "顯示Claude Code安裝",
            "Auto-retry in": "自動重試",
            "Colors:": "顏色:",
            "Theme": "主題",
            "Time-proportional warnings": "時間比例警告",
            "Authentication": "身份驗證",
            "Credentials path:": "憑證路徑:",
            "Full path to .credentials.json, or the folder containing it. The same folder is used for account info and token stats. Leave empty to use $CLAUDE_CONFIG_DIR, or ~/.claude when it is not set.": ".credentials.json 的完整路徑，或包含它的資料夾。同一資料夾也用於帳戶資訊和 Token 統計。留空則使用 $CLAUDE_CONFIG_DIR，未設定時使用 ~/.claude。",
            "Session refresh:": "工作階段重新整理:",
            "Automatically refresh expired session": "自動重新整理過期的工作階段",
            "Briefly reopens claude in the background (about 15 seconds, no visible window, no message sent) to refresh the session before prompting you to log in manually.": "在提示您手動登入之前，在背景短暫重新開啟 claude（約15秒，無可見視窗，不發送訊息）以重新整理工作階段。"
        }
    })

    readonly property string systemLanguage: {
        let locale = Qt.locale().name;
        return locale;
    }

    function getEffectiveLanguage() {
        if (currentLanguage === "system") {
            let sysLang = systemLanguage;
            if (strings[sysLang]) {
                return sysLang;
            }
            let langCode = sysLang.split("_")[0];
            for (let key in strings) {
                if (key.startsWith(langCode + "_")) {
                    return key;
                }
            }
            return "en_US";
        }
        return currentLanguage;
    }

    function tr(text) {
        let v = version;
        let lang = getEffectiveLanguage();

        if (strings[lang] && strings[lang][text]) {
            return strings[lang][text];
        }
        if (strings["en_US"] && strings["en_US"][text]) {
            return strings["en_US"][text];
        }
        return text;
    }
}
